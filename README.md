...
1. Read [Setup and Workflow](docs/development/setup.md) and the
   [Documentation Policy](docs/development/documentation-policy.md). Coding agents start with
   [AGENTS.md](AGENTS.md).
2. Install the git hooks (optional): `./scripts/install-git-hooks.sh`.
3. Run `make test` (formatting, clippy, tests, operator report drift check).
4. For operation or converter changes, run the WPT cases and regenerate baselines:
   `make test-wpt-op OP=<name>`, then `make wpt-sync-onnx`, `make wpt-sync-litert`,
   `make wpt-sync-coreml` (macOS) or `make wpt-sync-trtx` (NVIDIA GPU). Review the diff.
5. Update the documentation that describes the changed behaviour, then open a pull request.
