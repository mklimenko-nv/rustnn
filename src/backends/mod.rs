...
/// Disabled LiteRT backend (built without `litert-runtime`); the policy helpers report nothing
/// as unsupported.
#[cfg(not(feature = "litert-runtime"))]
pub mod litert {
    pub(crate) use crate::backends::DisabledContext as LiteRtContext;

    /// Operations the LiteRT backend rejects; empty when the backend is disabled.
    pub fn unsupported_ops() -> &'static [&'static str] {
        &[]
    }

    /// Data type policy of the LiteRT backend; always `false` when the backend is disabled.
